...
*Ashes of Erebus*. Ships as a single portable `.exe` plus a single JSON
config/save file (`aoe_launcher_config.json`) written next to it. No installer,
no registry entries, no other footprint.
...
yet**. Treat it as a solid draft that may need a short debugging pass on a
real machine. Send me any build error or odd behavior and I'll fix it
...
- **Import/export via a compact share code**, formatted as
  ...
  itself is a small hand-rolled binary format (not JSON), gzip-compressed
...
AoELauncher/
  ...
  Core/
    ConfigManager.cs          <- load/save the JSON file (always next to the exe)
...
3. Right-click the project → **Publish** → Folder, targeting `win-x64`,
   self-contained, single file. This produces one `.exe` (60-150 MB, since
   it bundles the .NET runtime -- no separate runtime install needed on the
...
4. Drop that one `.exe` into the `Ashes of Erebus` mod folder itself (next
   ...
   everything else. The first run creates `aoe_launcher_config.json` right
   beside it.
